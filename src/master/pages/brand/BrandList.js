...
function BrandList() {
  ...
  const handleDelete = async (id) => {
    const response = await deleteItem('brand', id);
  ...
  const handleSubmit = async (data) => {
    ...
    try{
      if (editData) {
        // Update operation
        response = await updateItem('brand', editData.id, data);
      } else {
        // Create operation
        response = await createItem('brand', data);
      }
  ...
  const toggleActive = async (id) => {
    ...
    try {
      ...
      const response = await partialupdateItem('brand', id, { is_active: !updatedItem.is_active });
      setApiResponse(response)
    ...
    <div className="table-container">
      <h2 className="header-list">
        ...
        <div className="header-right">
          ...
          <button onClick={() => handleOpenPopup()} className="add-button">Add New Brand</button>
        </div>
      </h2>
